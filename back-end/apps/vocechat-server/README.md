...
## API authentication (this fork)

- **System administrator**: On startup, if no user has `is_admin`, the server inserts **uid `1`**, **`name` = `admin`**, `is_admin = true`. SQLite autoincrement then assigns **uid ≥ 2** to everyone created via `POST /api/user/register` or admin user creation. There is **only one** administrator; other users cannot be promoted to admin via the API.
- **Protected HTTP APIs**: send header `X-Acting-Uid` with the numeric internal user id (e.g. `2` for the first registered user after the system admin). Password login, magic links, OAuth/OIDC/JWT, token refresh, and related routes are **removed**.
- **Registration**: `POST /api/user/register` never grants admin; `name`, `email`, and `password` may all be omitted or empty (display `name` is set to the assigned uid string). When `disallow_user_registration` is enabled in organization settings, registration is blocked once any non-guest user exists (including the system admin).
- **`POST /admin/system/create_admin`**: Returns **410 Gone**; use the bootstrap admin **uid `1`** instead.
- **Bots**: admin-created bot API keys are unchanged (separate from `X-Acting-Uid`).

### Database note (upgrades)

If an existing database has **uid `1` occupied by a non-admin** and there is still **no** admin user, the server will **refuse to start** until you fix or replace that row (or restore from backup / use a fresh `db.sqlite` for a clean install).
