...
/// Loopback / RFC1918 / typical LAN — VoceChat license lists rarely include these hosts.
fn site_host_skips_domain_allowlist(domain: &str) -> bool {
    if matches!(domain, "localhost" | "127.0.0.1" | "::1") {
        return true;
    }
    let Ok(addr) = domain.parse::<std::net::IpAddr>() else {
        return false;
    };
    match addr {
        std::net::IpAddr::V4(v4) => v4.is_loopback() || v4.is_private() || v4.is_link_local(),
        // `is_unique_local` / link-local helpers need newer `std`; loopback covers typical dev.
        std::net::IpAddr::V6(v6) => v6.is_loopback(),
    }
}

/// Site host for license checks: prefers `Referer`, then `Origin` (browser fetch often omits Referer cross-origin).
#[inline(always)]
pub fn get_referer_domain(req: &Request) -> Option<String> {
    ...
    let origin = req.header("Origin").unwrap_or_default();
    if !origin.is_empty() {
...
/// Load license from disk, or if `[system] disable_license = true`, skip file I/O and allow unlimited users.
pub async fn init_license(state: &State) -> Result<()> {
    if state.config.system.disable_license {
        G_LICENSE.lock().await.user_limit = u32::MAX;
        return Ok(());
    }
    load_license(state).await
}
...
pub async fn check_license(state: &State, req: &Request) -> Result<()> {
    ...
    // Some browsers / webviews can suppress both Referer and Origin for cross-origin requests.
    // For self-hosted Memos-style flows, don't hard-block in that case.
    let Some(domain) = get_referer_domain(req) else {
        return Ok(());
    };
    let license_guard = G_LICENSE.lock().await;
    let license = license_guard.deref();
    if site_host_skips_domain_allowlist(domain.as_str()) {
        return Ok(());
    }
