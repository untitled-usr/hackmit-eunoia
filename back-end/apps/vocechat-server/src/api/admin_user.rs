...
impl ApiAdminUser {
    ...
    async fn update(
        ...
        if let Some(want_admin) = req.is_admin {
            if want_admin && uid.0 != crate::bootstrap_admin::SYSTEM_ADMIN_UID {
                return Err(Error::from_status(StatusCode::FORBIDDEN));
            }
            if !want_admin && uid.0 == crate::bootstrap_admin::SYSTEM_ADMIN_UID {
                return Err(Error::from_status(StatusCode::FORBIDDEN));
        ...
        let sql = "insert into user_log (uid, action, name, gender, is_admin, language) values (?, ?, ?, ?, ?, ?)";
...
mod tests {
    ...
    async fn test_create_user() {
        ...
        let acting = server.login(uid).await;
        assert_eq!(uid.to_string(), acting);
    }
    ...
    async fn test_update_user_info() {
        ...
        let resp = server
            ...
            .header("X-Acting-Uid", &admin_token)
            .body_json(&json!({ "name": "customname", "gender": 2 }))
            .send()
            .await;
        resp.assert_status_is_ok();
        ...
        json.value().object().get("name").assert_string("customname");
    ...
    async fn test_update_user_password() {
        ...
        let admin_token = server.login_admin().await;
        let uid1 = server.create_user(&admin_token).await;
        let user_acting = uid1.to_string();

        server
            .post("/api/user/change_password")
            .header("X-Acting-Uid", &user_acting)
            .body_json(&json!({
                "old_password": "123456",
                "new_password": "123456",
            }))
