...
async fn resolve_uid_by_acting_uid_header(state: &State, acting_uid: Option<&str>) -> Result<i64> {
    let uid = acting_uid
...
impl ApiBot {
    ...
    async fn get_related_groups(
        &self,
        state: Data<&State>,
        #[oai(name = "X-Acting-Uid")] acting_uid: Header<Option<String>>,
        public_only: Query<Option<bool>>,
    ) -> Result<Json<Vec<Group>>> {
        let current_uid =
            resolve_uid_by_acting_uid_header(&state, acting_uid.0.as_deref()).await?;
        let cache = state.cache.read().await;
