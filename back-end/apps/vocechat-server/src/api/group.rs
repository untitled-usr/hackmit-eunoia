...
mod tests {
    ...
    async fn test_send_to_private_group() {
        ...
        for _ in 1..10 {
            let uid = server.create_user(&admin_token).await;
            ids.push(uid);
            tokens.push(server.login(uid).await);
        }

        // create group
        let resp = server
            .post("/api/group")
            .header("X-Acting-Uid", &tokens[0])
            .body_json(&json!({
                "name": "test",
    ...
    //     let resp = server
    //         .get(format!("/api/group/create_reg_magic_link?gid=", gid))
    //         .header("X-Acting-Uid", &user1_token)
    //         .send()
    //         .await;
    //     resp.assert_status_is_ok();
    //
    //     // create invite link by a user not in the group
    //     server.create_user(&admin_token).await;
    //     let user2_token = server.login(uid2).await;
    //
    //     let resp = server
    //         .get(format!("/api/group/create_reg_magic_link?gid=", gid))
    //         .header("X-Acting-Uid", &user2_token)
    //         .send()
    //         .await;
    //     resp.assert_status(StatusCode::FORBIDDEN);
    // }
    ...
    async fn received_pinned_message_updated_private_group() {
        ...
        resp.assert_status_is_ok();
        let json = resp.json().await;
        let pinned = json.value().object().get("pinned_messages").array();
        pinned.assert_len(2);
        pinned.get(0).object().get("mid").assert_i64(mid1);
        pinned.get(0).object().get("content").assert_string("a");
        pinned.get(1).object().get("mid").assert_i64(mid2);
        pinned.get(1).object().get("content").assert_string("b");

        let resp = server
            .post(format!("/api/group/{}/unpin", gid))
            .header("X-Acting-Uid", &admin_token)
            .body_json(&json!({ "mid": mid1 }))
            .send()
            .await;
        resp.assert_status_is_ok();
        ...
        let pinned = json.value().object().get("pinned_messages").array();
        pinned.assert_len(1);
        pinned.get(0).object().get("mid").assert_i64(mid2);
        pinned.get(0).object().get("content").assert_string("b");
    }
