...
/// Friend request snapshot (SSE / settings)
#[derive(Debug, Object, Clone)]
...
/// Contact graph sync for clients (`added` | `blocked` | empty string)
#[derive(Debug, Object, Clone)]
pub struct ContactRelationSync {
    pub target_uid: i64,
    pub status: String,
    #[oai(default)]
...
pub struct UserSettingsMessage {
    ...
    #[oai(default)]
    pub incoming_friend_requests: Vec<FriendRequestView>,
    #[oai(default)]
    pub outgoing_friend_requests: Vec<FriendRequestView>,
    #[oai(default)]
...
pub struct UserSettingsChangedMessage {
    ...
    pub burn_after_reading_groups: Vec<BurnAfterReadingGroup>,
    #[oai(default)]
    pub friend_requests: Vec<FriendRequestView>,
    #[oai(default)]
    pub add_blocked_users: Vec<i64>,
    #[oai(default)]
    pub remove_blocked_users: Vec<i64>,
    #[oai(default)]
    pub contact_updates: Vec<ContactRelationSync>,
}
...
pub async fn send_message(state: &State, mut payload: ChatMessagePayload) -> poem::Result<i64> {
    ...
    let mid = match payload.target {
        MessageTarget::User(MessageTargetUser { uid }) => {
            if let Some(target_user) = cache.users.get(&uid) {
                if target_user.has_blocked(from_uid) {
