...
use crate::{
    api::{tags::ApiTags, token::Token, SmtpConfig},
    config::Config,
...
pub struct OrganizationConfig {
    ...
    /// When true, `POST /user/register` rejects new users once any non-guest user exists (including the uid=1 system admin).
    #[serde(default)]
    #[oai(default)]
...
impl ApiAdminSystem {
    ...
    /// Deprecated: the uid=1 system admin is created automatically at server startup.
    #[oai(path = "/create_admin", method = "post")]
    async fn create_admin_user(&self) -> Result<PlainText<&'static str>> {
        Err(Error::from_status(StatusCode::GONE))
    }
