...
pub use message::{
    ...
    ChatMessagePayload, ContactRelationSync, FriendRequestView, GroupChangedMessage,
    HeartbeatMessage, JoinedGroupMessage, KickFromGroupMessage, KickFromGroupReason, KickMessage,
    KickReason, Message, MessageDetail, MessageTarget, MessageTargetGroup, MessageTargetUser,
    MuteGroup, MuteUser, ReadIndexGroup, ReadIndexUser, RelatedGroupsMessage,
    UserJoinedGroupMessage, UserLeavedGroupMessage, UserSettingsChangedMessage,
    UserSettingsMessage, UserState, UserStateChangedMessage, UserUpdateLog, UsersStateMessage,
...
pub fn create_api_service() -> OpenApiService<impl OpenApi, ()> {
    // poem-openapi tuples are limited; nest to add `ApiSocial`.
    OpenApiService::new(
        (
            (
                token::ApiToken,
