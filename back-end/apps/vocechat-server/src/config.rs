...
pub struct Config {
    ...
    /// Deprecated: ignored at runtime; kept so older TOML `[template]` tables still deserialize.
    #[serde(default)]
    pub template: Option<toml::Value>,
...
pub struct SystemConfig {
    ...
    /// When true, skip reading/writing `data_dir/license`, ignore user/domain/expiry license checks (self-hosted).
    #[serde(default)]
    pub disable_license: bool,
    /// When true, do not insert the uid=1 system admin (integration tests only).
    #[serde(default)]
    pub test_skip_system_admin_bootstrap: bool,
}
