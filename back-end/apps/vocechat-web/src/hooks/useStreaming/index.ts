...
export default function useStreaming() {
  ...
  const startStreaming = useCallback(async () => {
    ...
    if (!actingUid) {
      console.info("debug SSE: actingUid empty");
      return;
    }
    const params: {
      limit: string;
        ...
        case "user_settings_changed": {
          Object.keys(data).forEach((key) => {
            switch (key) {
              ...
              case "blocked_users":
                {
                  const ev = data as UserSettingsEvent;
                  dispatch(
                    setSocialFromUserSettings({
                      blocked_users: ev.blocked_users,
                      incoming_friend_requests: ev.incoming_friend_requests,
                      outgoing_friend_requests: ev.outgoing_friend_requests,
                    })
                  );
                }
                break;
              case "friend_requests":
                {
                  const arr = (data as UserSettingsChangedEvent).friend_requests ?? [];
                  if (arr.length) {
                    dispatch(
                      mergeFriendRequests({
              ...
              case "add_blocked_users":
                {
                  const arr = (data as UserSettingsChangedEvent).add_blocked_users ?? [];
                  if (arr.length) dispatch(addBlockedUsers(arr));
                }
                break;
              case "remove_blocked_users":
                {
                  const arr = (data as UserSettingsChangedEvent).remove_blocked_users ?? [];
                  if (arr.length) dispatch(removeBlockedUsers(arr));
                }
                break;
              case "contact_updates":
                {
                  const arr = (data as UserSettingsChangedEvent).contact_updates ?? [];
                  if (arr.length) {
                    const mapped = arr.map((u) => {
                      let status: ContactStatus = "";
                      if (u.status === "added") status = "added";
                      else if (u.status === "blocked") status = "blocked";
                      return {
                        uid: u.target_uid,
                        status,
                        removed_by_peer: Boolean(u.removed_by_peer),
                      };
                    });
                    dispatch(updateContactStatus(mapped));
                  }
                }
                break;
