...
const useUserOperation = ({ uid, cid }: IProps) => {
  ...
  const { t: memT } = useTranslation("member");
  ...
  const copyUserId = (explicitUid?: number) => {
    const target = explicitUid ?? uid;
  ...
  const removeFromContact = async () => {
    if (!uid) return;
    try {
      await removeFriend(uid).unwrap();
      toast.success(ct("tip.delete"));
    } catch {
      try {
        await updateContactStatus({ target_uid: uid, action: "remove" }).unwrap();
        toast.success(ct("tip.delete"));
      } catch {
        toast.error(memT("action_failed", { defaultValue: "操作失败" }));
      }
    }
    hideAll();
  };
  ...
  /** 仅互为好友时展示「删除好友」 */
  const canRemoveFromContact: boolean =
    loginUid != uid && user?.status === "added";
  ...
  return {
    ...
    canCopyUserId: show_email && (typeof uid === "number" ? !!user : typeof cid === "number"),
    copyUserId,
    canDM: dm_to_member,
    startCall,
    canViewPassword,
