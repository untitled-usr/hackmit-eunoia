...
import { ContactInfo, FriendRequestDTO, User } from "./user";
...
export type ContactRelationSync = {
  target_uid: number;
  status: string;
...
export type UserSettingsChangedEvent = {
  ...
  burn_after_reading_users?: AutoDeleteMsgForUser[];
  burn_after_reading_groups?: AutoDeleteMsgForGroup[];
  friend_requests?: FriendRequestDTO[];
  add_blocked_users?: number[];
  remove_blocked_users?: number[];
  contact_updates?: ContactRelationSync[];
};
