      ...
      >
        <ViewportList initialPrerender={15} viewportRef={ref} items={finalUids}>
          {(uid) => {
            ...
            dotsVisible = [canCopyUserId, canRemove, canRemoveFromChannel].some((i) => i);
            return (
              <li
                key={uid}
                    ...
                    <Tippy
                      ...
                      content={
                        <ul className="min-w-30 context-menu">
                          {canCopyUserId && (
                            <li className="item" onClick={() => copyUserId(uid)}>
                              {ct("action.copy_uid", { defaultValue: "Copy user ID" })}
                            </li>
                          )}
