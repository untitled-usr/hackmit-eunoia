...
const usersSlice = createSlice({
  ...
  reducers: {
    ...
    /** Keep `users.byId` in sync with session after register / getMe (otherwise settings read stale or missing row). */
    upsertUser(state, action: PayloadAction<StoredUser>) {
      const u = action.payload;
      state.byId[u.uid] = { ...state.byId[u.uid], ...u };
      if (!state.ids.includes(u.uid)) {
        state.ids.push(u.uid);
      }
    },
    ...
    updateContactStatus(
      ...
      action: PayloadAction<
        | { uid: number; status: ContactStatus; removed_by_peer?: boolean }
        | { uid: number; status: ContactStatus; removed_by_peer?: boolean }[]
      >
    ) {
      ...
      arr.forEach((data) => {
        if (state.byId[data.uid]) {
          ...
          if (data.removed_by_peer !== undefined) {
            state.byId[data.uid]!.removed_by_peer = data.removed_by_peer;
          } else if (data.status === "" || data.status === "blocked") {
            state.byId[data.uid]!.removed_by_peer = false;
          }
        }
      });
    }
