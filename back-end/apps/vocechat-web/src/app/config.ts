...
const local_dev = `http://localhost:7922`;
// const local_dev = `https://dev.voce.chat`;

/**
 * 开发环境后端 origin（vocechat-server，默认端口 7922）。
 * 前端 API 始终面向本仓库的 Rust 服务，不经过 mid-auth；`REACT_APP_SERVER_*` 仅用于
 * 开发时直连回源（例如代理失败时的兜底），不要把 mid-auth 的 origin 填在这里。
 *
 * 通过局域网 IP 打开前端时，若仍写死 localhost 与错误端口，
 * 请求会打到「浏览器所在机器」而非开发机 → 表现为无法连上后端。
 * 可选覆盖：REACT_APP_SERVER_ORIGIN（完整 origin）或 REACT_APP_SERVER_PORT（DevStack 默认 7922）。
 */
function getDevBackendOrigin(): string {
  if (process.env.REACT_APP_OFFICIAL_DEMO) {
    return official_dev;
  }
  const fromEnv =
    process.env.REACT_APP_SERVER_ORIGIN?.trim() ||
  ...
  const port = process.env.REACT_APP_SERVER_PORT?.trim() || "7922";
  const scheme = process.env.REACT_APP_SERVER_SCHEME?.trim() || "http";
...
const getBaseOrigin = () => {
  ...
  const dev = getDevBackendOrigin();
  console.log("[Config] Using dev backend origin:", dev);
  return dev;
};
...
// In local dev, always use same-origin `/api` and let dev-server proxy to backend.
// This avoids mixed-content/CORS/network issues when browser cannot directly reach 7922.
const BASE_URL = process.env.NODE_ENV === "development" ? "/api" : `${BASE_ORIGIN}/api`;
export const DEV_DIRECT_BASE_URL = `${getDevBackendOrigin()}/api`;
...
/** Memos-style acting user header (matches vocechat-server `Token` security scheme). */
export const actingUidHeader = "X-Acting-Uid";
/** @deprecated Use actingUidHeader; kept for legacy imports. */
export const tokenHeader = actingUidHeader;
