...
export const messageApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    deleteMessages: builder.query<number, number[]>({
      queryFn: async (mids) => ({ data: mids[0] ?? 0 }),
        ...
        if (messages?.length) {
          ...
          const processBatch = (startIndex: number) => {
            ...
            batch(() => {
              ...
              const curr = {
                afterMid:
                  typeof window !== "undefined" && window.AFTER_MID !== undefined
                    ? window.AFTER_MID
                    : state.footprint.afterMid,
                ready: true,
                loginUid: state.authData.user?.uid ?? 0,
                readUsers: state.footprint.readUsers,
                readChannels: state.footprint.readChannels
              };
              for (let i = startIndex; i < endIndex; i++) {
                handleChatMessage(
                  { type: "chat", ...messages[i] } as ChatEvent,
                  dispatch,
                  curr,
                  fromHistory
                );
    ...
    >({
      ...
      async onQueryStarted(param1, param2) {
        const from = param1.context === "channel" ? "channel" : "user";
        await onMessageSendStarted.call(this, param1, param2, from);
      }
    }),
