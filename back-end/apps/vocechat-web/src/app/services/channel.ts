...
export const channelApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createInviteLink: builder.query<string, { expire: number; times: number }>({
      queryFn: async () => ({
        data: transformInviteLink("http://vocechat.local/fork-invite-not-supported")
      })
    }),
    ...
    >({
      queryFn: async () => ({
        data: transformInviteLink("http://vocechat.local/fork-private-invite-not-supported")
      })
    }),
    ...
    joinPrivateChannel: builder.mutation<Channel, { magic_token: string }>({
      async queryFn() {
        return {
          error: {
            status: 501,
            data: "join_private_not_supported"
          }
        };
      }
    }),
