...
import NewFriendsCenter from "./NewFriendsCenter";
...
function UsersPage() {
  ...
  // `/users/new-friends` 是独立子路由，不会填充 `:user_id`，不能靠 useParams 判断
  const isNewFriends =
    pathname === "/users/new-friends" || pathname.endsWith("/users/new-friends");
  // 记住路由
  useEffect(() => {
  ...
  const isUserDetail = !!user_id || isNewFriends;
  return (
