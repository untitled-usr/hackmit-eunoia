...
func checkMemoAccess(ctx context.Context, friendResolver friendship.Resolver, memo *store.Memo, userID int32) error {
...
func applyVisibilityFilter(ctx context.Context, friendResolver friendship.Resolver, find *store.FindMemo, userID int32) {
	if userID == 0 {
		find.VisibilityList = []store.Visibility{store.Public}
	} else {
		friendIDs := []int32{}
		if friendResolver != nil && friendResolver.Enabled() {
			ids, err := friendResolver.ListFriendIDs(ctx, userID)
			if err == nil {
				friendIDs = ids
			}
		}
		find.Filters = append(find.Filters, friendship.BuildFriendFilterExpr(userID, friendIDs))
	}
}
...
func parseVisibility(s string) (store.Visibility, error) {
	...
	case store.Public, store.Private:
		return v, nil
	default:
...
func extractUserID(ctx context.Context) (int32, error) {
	...
	if id == 0 {
		return 0, errors.New("unauthenticated: X-Acting-Uid header is required")
	}
	return id, nil
}
...
func (s *MCPService) registerMemoTools(mcpSrv *mcpserver.MCPServer) {
	mcpSrv.AddTool(mcp.NewTool("list_memos",
		mcp.WithDescription("List memos visible to the caller. Authenticated users see their own memos and public memos; unauthenticated callers see only public memos."),
	...
	mcpSrv.AddTool(mcp.NewTool("search_memos",
		mcp.WithDescription("Search memo content. Authenticated users search their own memos and public memos; unauthenticated callers search public memos only."),
...
func (s *MCPService) handleListMemos(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	applyVisibilityFilter(ctx, s.friendResolver, find, userID)
...
func (s *MCPService) handleGetMemo(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err := checkMemoAccess(ctx, s.friendResolver, memo, userID); err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
...
func (s *MCPService) handleListMemoComments(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err := checkMemoAccess(ctx, s.friendResolver, parent, userID); err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	...
	for _, m := range memos {
		if checkMemoAccess(ctx, s.friendResolver, m, userID) == nil {
