...
Requests may include an acting user header:

```
X-Acting-Uid: <user-id>
```

Behavior:
- Missing header: treated as anonymous.
- Invalid header value: `HTTP 401`.
- Write tools still require a valid `X-Acting-Uid` (no bearer tokens or AuthService in this fork).
