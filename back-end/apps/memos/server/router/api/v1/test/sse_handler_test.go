...
func TestSSEHandler_Authentication(t *testing.T) {
	...
	t.Run("no acting uid returns 200", func(t *testing.T) {
	...
	t.Run("invalid acting uid returns 401", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/v1/sse", nil)
		req.Header.Set("X-Acting-Uid", "invalid")
		rec := httptest.NewRecorder()
		e.ServeHTTP(rec, req)
		require.Equal(t, http.StatusUnauthorized, rec.Code)
	})

	t.Run("valid acting uid returns 200 and stream", func(t *testing.T) {
		...
		defer cancel()
		req := httptest.NewRequest(http.MethodGet, "/api/v1/sse", nil).WithContext(reqCtx)
		req.Header.Set("X-Acting-Uid", strconv.Itoa(int(user.ID)))
		rec := httptest.NewRecorder()
		done := make(chan struct{})
		go func() {
			defer close(done)
	...
	t.Run("token in query param is ignored", func(t *testing.T) {
		reqCtx, cancel := context.WithCancel(context.Background())
		defer cancel()
		req := httptest.NewRequest(http.MethodGet, "/api/v1/sse?token=legacy", nil).WithContext(reqCtx)
		rec := httptest.NewRecorder()
		done := make(chan struct{})
		go func() {
			defer close(done)
