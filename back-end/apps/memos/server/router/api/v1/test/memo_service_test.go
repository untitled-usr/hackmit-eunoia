...
func TestListMemos(t *testing.T) {
	...
	require.Equal(t, apiv1.Visibility_PRIVATE, memoOneRes.GetVisibility())
	...
	// memoTwo is PUBLIC and belongs to another user, so it should still be visible.
	memoTwoResIdx := slices.IndexFunc(memos.Memos, func(m *apiv1.Memo) bool { return m.GetName() == memoTwo.GetName() })
	require.NotEqual(t, -1, memoTwoResIdx)
	...
	require.Equal(t, apiv1.Visibility_PUBLIC, memoTwoRes.GetVisibility())
	...
	require.Equal(t, apiv1.Visibility_PUBLIC, memoThreeRes.GetVisibility())
...
func TestPrivateMemoRestrictedToCreator(t *testing.T) {
	ctx := context.Background()
	...
	_, err = ts.Service.CreateMemoComment(ownerCtx, &apiv1.CreateMemoCommentRequest{
		Name: memo.Name,
		Comment: &apiv1.Memo{
			Content:    "Owner-only comment",
			Visibility: apiv1.Visibility_PRIVATE,
		},
	})
	require.NoError(t, err)

	_, err = ts.Service.GetMemo(viewerCtx, &apiv1.GetMemoRequest{Name: memo.Name})
	require.Error(t, err)
	require.Contains(t, err.Error(), "permission denied")

	listResp, err := ts.Service.ListMemos(viewerCtx, &apiv1.ListMemosRequest{
		PageSize: 10,
		Filter:   fmt.Sprintf("creator_id == %d", owner.ID),
	})
	require.NoError(t, err)
	require.Empty(t, listResp.Memos)

	_, err = ts.Service.ListMemoComments(viewerCtx, &apiv1.ListMemoCommentsRequest{Name: memo.Name})
	require.Error(t, err)
	require.Contains(t, err.Error(), "permission denied")

	_, err = ts.Service.ListMemoAttachments(viewerCtx, &apiv1.ListMemoAttachmentsRequest{Name: memo.Name})
	require.Error(t, err)
	require.Contains(t, err.Error(), "permission denied")

	_, err = ts.Service.ListMemoReactions(viewerCtx, &apiv1.ListMemoReactionsRequest{Name: memo.Name})
	require.Error(t, err)
	require.Contains(t, err.Error(), "permission denied")

	_, err = ts.Service.ListMemoRelations(viewerCtx, &apiv1.ListMemoRelationsRequest{Name: memo.Name})
	require.Error(t, err)
	require.Contains(t, err.Error(), "permission denied")

	stats, err := ts.Service.GetUserStats(viewerCtx, &apiv1.GetUserStatsRequest{Name: fmt.Sprintf("users/%d", owner.ID)})
	require.NoError(t, err)
	require.Equal(t, int32(0), stats.TotalMemoCount)
}

func TestPublicMemoVisibleToFriend(t *testing.T) {
	ctx := context.Background()

	ts := NewTestService(t)
	defer ts.Cleanup()

	owner, err := ts.CreateRegularUser(ctx, "owner-friend")
	require.NoError(t, err)
	ownerCtx := ts.CreateUserContext(ctx, owner.ID)

	viewer, err := ts.CreateRegularUser(ctx, "viewer-friend")
	require.NoError(t, err)
	viewerCtx := ts.CreateUserContext(ctx, viewer.ID)

	ts.SetupFriendshipDB(t, FriendshipLink{
		UserA: owner.ID,
		UserB: viewer.ID,
	})
	...
	memo, err := ts.Service.CreateMemo(ownerCtx, &apiv1.CreateMemoRequest{
		Memo: &apiv1.Memo{
			Content:    "Owner public memo visible to friend",
			Visibility: apiv1.Visibility_PUBLIC,
		},
	})
	require.NoError(t, err)
	...
	_, err = ts.Service.CreateAttachment(ownerCtx, &apiv1.CreateAttachmentRequest{
		Attachment: &apiv1.Attachment{
			Filename: "friend-visible.txt",
			Type:     "text/plain",
			Content:  []byte("friend-only"),
			Memo:     &memo.Name,
		},
	})
	require.NoError(t, err)
	...
	_, err = ts.Service.CreateMemoComment(ownerCtx, &apiv1.CreateMemoCommentRequest{
		Name: memo.Name,
		Comment: &apiv1.Memo{
			Content:    "Friend can read this comment",
			Visibility: apiv1.Visibility_PUBLIC,
		},
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Len(t, listResp.Memos, 1)
	require.Equal(t, memo.Name, listResp.Memos[0].Name)
...
func TestPrivateMemoHiddenWhenFriendshipSoftDeleted(t *testing.T) {
	ctx := context.Background()

	ts := NewTestService(t)
	defer ts.Cleanup()

	owner, err := ts.CreateRegularUser(ctx, "owner-deleted")
	require.NoError(t, err)
	ownerCtx := ts.CreateUserContext(ctx, owner.ID)

	viewer, err := ts.CreateRegularUser(ctx, "viewer-deleted")
	require.NoError(t, err)
	viewerCtx := ts.CreateUserContext(ctx, viewer.ID)
	...
	memo, err := ts.Service.CreateMemo(ownerCtx, &apiv1.CreateMemoRequest{
		Memo: &apiv1.Memo{
			Content:    "Owner private memo hidden after friendship deleted",
			Visibility: apiv1.Visibility_PRIVATE,
		},
	})
	require.NoError(t, err)
...
func TestCreateAndUpdateMemoRejectRemovedProtectedVisibility(t *testing.T) {
	ctx := context.Background()

	ts := NewTestService(t)
	defer ts.Cleanup()

	owner, err := ts.CreateRegularUser(ctx, "owner-invalid-visibility")
	require.NoError(t, err)
	ownerCtx := ts.CreateUserContext(ctx, owner.ID)
