...
func (s *APIV1Service) canReadMemo(ctx context.Context, memo *store.Memo, user *store.User) bool {
...
func (s *APIV1Service) buildReadMemoFilterForUser(ctx context.Context, userID int32) string {
	return friendship.BuildFriendFilterExpr(userID, nil)
}

func (s *APIV1Service) getVisibilityListForViewer(ctx context.Context, viewer *store.User, creatorID int32) []store.Visibility {
	if viewer == nil {
		return []store.Visibility{store.Public}
	}
	if viewer.ID == creatorID || isSuperUser(viewer) {
		return nil
	}
	return []store.Visibility{store.Public}
}
