...
func (s *APIV1Service) CreateMemo(ctx context.Context, request *v1pb.CreateMemoRequest) (*v1pb.Memo, error) {
	...
	if request.Memo.Visibility != v1pb.Visibility_VISIBILITY_UNSPECIFIED &&
		request.Memo.Visibility != v1pb.Visibility_PRIVATE &&
		request.Memo.Visibility != v1pb.Visibility_PUBLIC {
		return nil, status.Errorf(codes.InvalidArgument, "invalid visibility: only PRIVATE or PUBLIC are supported")
	}
...
func (s *APIV1Service) ListMemos(ctx context.Context, request *v1pb.ListMemosRequest) (*v1pb.ListMemosResponse, error) {
	...
	if currentUser == nil {
		...
		if memoFind.CreatorID == nil {
			memoFind.Filters = append(memoFind.Filters, s.buildReadMemoFilterForUser(ctx, currentUser.ID))
		} else if *memoFind.CreatorID != currentUser.ID {
			memoFind.VisibilityList = s.getVisibilityListForViewer(ctx, currentUser, *memoFind.CreatorID)
		}
	}
...
func (s *APIV1Service) GetMemo(ctx context.Context, request *v1pb.GetMemoRequest) (*v1pb.Memo, error) {
	...
	if !s.canReadMemo(ctx, memo, user) {
		if user == nil {
			return nil, status.Errorf(codes.Unauthenticated, "user not authenticated")
		}
...
func (s *APIV1Service) UpdateMemo(ctx context.Context, request *v1pb.UpdateMemoRequest) (*v1pb.Memo, error) {
	...
	for _, path := range request.UpdateMask.Paths {
		if path == "content" {
			...
			if request.Memo.Visibility != v1pb.Visibility_PRIVATE && request.Memo.Visibility != v1pb.Visibility_PUBLIC {
				return nil, status.Errorf(codes.InvalidArgument, "invalid visibility: only PRIVATE or PUBLIC are supported")
			}
			instanceMemoRelatedSetting, err := s.Store.GetInstanceMemoRelatedSetting(ctx)
			if err != nil {
...
func (s *APIV1Service) CreateMemoComment(ctx context.Context, request *v1pb.CreateMemoCommentRequest) (*v1pb.Memo, error) {
	...
	if !s.canReadMemo(ctx, relatedMemo, user) {
		return nil, status.Errorf(codes.PermissionDenied, "permission denied")
	}
...
func (s *APIV1Service) ListMemoComments(ctx context.Context, request *v1pb.ListMemoCommentsRequest) (*v1pb.ListMemoCommentsResponse, error) {
	...
	if !s.canReadMemo(ctx, memo, currentUser) {
		if currentUser == nil {
			return nil, status.Errorf(codes.Unauthenticated, "user not authenticated")
		}
		return nil, status.Errorf(codes.PermissionDenied, "permission denied")
	...
	if currentUser == nil {
		...
		memoFilter = s.buildReadMemoFilterForUser(ctx, currentUser.ID)
	}
...
func (*APIV1Service) parseMemoOrderBy(orderBy string, memoFind *store.FindMemo) error {
	...
	for _, field := range fields {
		...
		case "diary_order":
			memoFind.OrderByDiaryOrder = true
		...
		case "update_time":
			...
			memoFind.OrderByTimeAsc = fieldDirection == "asc"
		default:
			return errors.Errorf("unsupported order field: %s, supported fields are: pinned, diary_order, display_time, create_time, update_time, name", fieldName)
		}
	}
