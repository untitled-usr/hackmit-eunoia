...
const (
	...
	DriftBottleNamePrefix      = "drift-bottles/"
...
// ExtractDriftBottleUIDFromName returns the drift bottle UID from a resource name.
// e.g., "drift-bottles/uuid" -> "uuid".
func ExtractDriftBottleUIDFromName(name string) (string, error) {
	tokens, err := GetNameParentTokens(name, DriftBottleNamePrefix)
	if err != nil {
		return "", err
	}
	return tokens[0], nil
