...
// getCurrentUser retrieves the acting user from X-Acting-Uid header.
// Returns nil when the header is not provided.
func (s *FileServerService) getCurrentUser(ctx context.Context, c *echo.Context) (*store.User, error) {
	result, err := s.authenticator.AuthenticateByActingUIDHeader(ctx, c.Request().Header.Get("X-Acting-Uid"))
	if err != nil {
		return nil, err
	}
