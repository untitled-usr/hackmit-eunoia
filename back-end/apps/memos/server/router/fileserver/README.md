...
- Optionally resolve acting user from `X-Acting-Uid`
...
**Acting user:** `X-Acting-Uid` may be required for non-public memos (no bearer login in this fork).
...
The fileserver can resolve an acting user from:

1. **Acting UID Header** (`X-Acting-Uid: {uid}`)
   - Positive integer user ID
   - Invalid value returns unauthorized on protected resources
   - Missing header keeps request anonymous

### Authentication Flow

```
Request → getCurrentUser()
    ├─→ Parse X-Acting-Uid header
    ├─→ Get user from DB
    └─→ Return user (if valid) / nil (if missing)
```
...
Resolves request user from `X-Acting-Uid`.
...
### 2. Authentication
Private content requires a valid acting user context.
