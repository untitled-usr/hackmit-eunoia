...
import (
	...
	"github.com/usememos/memos/server/runner/driftrefresh"
...
func NewServer(ctx context.Context, profile *profile.Profile, store *store.Store) (*Server, error) {
	...
	friendResolver, err := friendship.NewSQLiteResolver(profile.FriendshipDSN)
	if err != nil {
		return nil, errors.Wrap(err, "failed to initialize friendship resolver")
	}
	s.friendResolver = friendResolver
	...
	apiV1Service := apiv1.NewAPIV1Service(s.Secret, profile, store, s.friendResolver)
	...
	// Register MCP server.
	mcpService := mcprouter.NewMCPService(s.Profile, s.Store, s.Secret, s.friendResolver)
...
func (s *Server) Shutdown(ctx context.Context) {
	...
	if s.friendResolver != nil {
		if err := s.friendResolver.Close(); err != nil {
			slog.Error("failed to close friendship resolver", slog.String("error", err.Error()))
		}
	}
...
func (s *Server) StartBackgroundRunners(ctx context.Context) {
	...
	driftRefreshRunner := driftrefresh.NewRunner(s.Store)
	driftRefreshRunner.RunOnce(ctx)
	go func() {
		driftRefreshRunner.Run(driftContext)
		slog.Info("driftrefresh runner stopped")
	}()
