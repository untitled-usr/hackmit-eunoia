    ...
    <div className="mx-auto flex max-w-2xl gap-4 sm:gap-6">
      ...
      <div className="flex flex-1 flex-col gap-3">
        <div>
          ...
          {user.displayName && user.displayName !== user.username && <p className="text-sm text-muted-foreground">{user.username}</p>}
        </div>
        {(user.gender || (user.age ?? 0) > 0) && (
          <p className="text-sm text-muted-foreground">
            {[user.gender, (user.age ?? 0) > 0 ? `${user.age}` : ""].filter(Boolean).join(" · ")}
          </p>
        )}
