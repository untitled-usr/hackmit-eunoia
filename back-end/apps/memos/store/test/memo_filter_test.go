...
func TestMemoFilterVisibilityEquals(t *testing.T) {
	...
	tc.CreateMemo(NewMemoBuilder("memo-private-2", tc.User.ID).Content("Private memo 2").Visibility(store.Private))
...
func TestMemoFilterVisibilityInList(t *testing.T) {
	...
	tc.CreateMemo(NewMemoBuilder("memo-priv-2", tc.User.ID).Visibility(store.Private))
...
func TestMemoFilterLogicalOr(t *testing.T) {
	...
	tc.CreateMemo(NewMemoBuilder("memo-private-2", tc.User.ID).Visibility(store.Private))
...
func TestMemoFilterDiaryStatus(t *testing.T) {
	t.Parallel()
	tc := NewMemoFilterTestContext(t)
	defer tc.Close()

	tc.CreateMemo(NewMemoBuilder("memo-diary-normal", tc.User.ID).
		Content("Diary normal").
		Location("normal", 0, 10))
	tc.CreateMemo(NewMemoBuilder("memo-diary-digested", tc.User.ID).
		Content("Diary digested").
		Location("digested", 0, 20))

	memos := tc.ListWithFilter(`diary_status == "digested"`)
	require.Len(t, memos, 1)
	require.Equal(t, "memo-diary-digested", memos[0].UID)
}

func TestMemoFilterDiaryUnlockTs(t *testing.T) {
	t.Parallel()
	tc := NewMemoFilterTestContext(t)
	defer tc.Close()

	tc.CreateMemo(NewMemoBuilder("memo-diary-locked", tc.User.ID).
		Content("Diary locked").
		Location("normal", 4102444800, 10))
	tc.CreateMemo(NewMemoBuilder("memo-diary-unlocked", tc.User.ID).
		Content("Diary unlocked").
		Location("normal", 0, 20))

	memos := tc.ListWithFilter(`diary_unlock_ts > 0`)
	require.Len(t, memos, 1)
	require.Equal(t, "memo-diary-locked", memos[0].UID)
}

func TestMemoFilterDiaryOrder(t *testing.T) {
	t.Parallel()
	tc := NewMemoFilterTestContext(t)
	defer tc.Close()

	tc.CreateMemo(NewMemoBuilder("memo-diary-order-1", tc.User.ID).
		Content("Diary order 1").
		Location("normal", 0, 1))
	tc.CreateMemo(NewMemoBuilder("memo-diary-order-2", tc.User.ID).
		Content("Diary order 2").
		Location("normal", 0, 2))

	memos := tc.ListWithFilter(`diary_order >= 2`)
	require.Len(t, memos, 1)
	require.Equal(t, "memo-diary-order-2", memos[0].UID)
}
