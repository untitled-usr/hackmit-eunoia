...
func TestMemoListByVisibility(t *testing.T) {
	...
	// List private memos only
	privateMemos, err := ts.ListMemos(ctx, &store.FindMemo{
		VisibilityList: []store.Visibility{store.Private},
	})
	require.NoError(t, err)
	require.Equal(t, 2, len(privateMemos))
	require.Equal(t, store.Private, privateMemos[0].Visibility)

	// List public and private (multiple visibility)
	publicAndPrivate, err := ts.ListMemos(ctx, &store.FindMemo{
		VisibilityList: []store.Visibility{store.Public, store.Private},
	})
	require.NoError(t, err)
	require.Equal(t, 3, len(publicAndPrivate))
...
func TestMemoUpdateVisibility(t *testing.T) {
	...
	found, err = ts.GetMemo(ctx, &store.FindMemo{ID: &memo.ID})
	require.NoError(t, err)
	require.Equal(t, store.Public, found.Visibility)
