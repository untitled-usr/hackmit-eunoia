...
func TestUserStore(t *testing.T) {
	...
	require.Equal(t, store.BuiltinAdminID, admin.ID)
	...
	admin, err = ts.UpdateUser(ctx, &store.UpdateUser{
		ID:       admin.ID,
		Nickname: &userPatchNickname,
	})
	require.NoError(t, err)
	require.Equal(t, userPatchNickname, admin.Nickname)

	extra, err := ts.CreateUser(ctx, &store.User{
		Username:     "extra-user-crud",
		Role:         store.RoleUser,
		Nickname:     "extra",
		PasswordHash: "",
	})
	require.NoError(t, err)
	err = ts.DeleteUser(ctx, &store.DeleteUser{ID: extra.ID})
	require.NoError(t, err)
...
func TestUserGetByUsername(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, found)
	require.Equal(t, store.BuiltinAdminUsername, found.Username)
...
func TestUserUpdateAllFields(t *testing.T) {
	...
	require.Equal(t, newGender, updated.Gender)
...
func TestUserListWithLimit(t *testing.T) {
	...
	// Builtin admin + 5 regular users
	for i := 0; i < 5; i++ {
		_, err := createTestingUserWithRole(ctx, ts, fmt.Sprintf("user%d", i), store.RoleUser)
		require.NoError(t, err)
	}
...
func createTestingHostUser(ctx context.Context, ts *store.Store) (*store.User, error) {
	...
	u, err := ts.GetUser(ctx, &store.FindUser{ID: &id})
	if err != nil {
		return nil, err
	}
	if u == nil {
		return nil, fmt.Errorf("builtin admin id=%d not found", store.BuiltinAdminID)
	}
	return u, nil
}
