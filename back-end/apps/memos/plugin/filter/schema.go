...
func NewSchema() Schema {
	fields := map[string]Field{
		...
		"diary_status": {
			...
			Column: Column{Table: "memo", Name: "payload"},
			Expressions: map[DialectName]string{
				DialectSQLite:   "COALESCE(CAST(json_extract(%s, '$.location.placeholder') AS TEXT), '')",
				DialectMySQL:    "COALESCE(JSON_UNQUOTE(JSON_EXTRACT(%s, '$.location.placeholder')), '')",
				DialectPostgres: "COALESCE(%s->'location'->>'placeholder', '')",
			},
			AllowedComparisonOps: map[ComparisonOperator]bool{
				CompareEq:  true,
				CompareNeq: true,
		...
		"diary_unlock_ts": {
			Name:   "diary_unlock_ts",
			Kind:   FieldKindScalar,
			Type:   FieldTypeTimestamp,
			Column: Column{Table: "memo", Name: "payload"},
			Expressions: map[DialectName]string{
				DialectSQLite:   "COALESCE(CAST(json_extract(%s, '$.location.latitude') AS INTEGER), 0)",
				DialectMySQL:    "COALESCE(CAST(JSON_UNQUOTE(JSON_EXTRACT(%s, '$.location.latitude')) AS SIGNED), 0)",
				DialectPostgres: "COALESCE((%s->'location'->>'latitude')::bigint, 0)",
			},
		},
		"diary_order": {
			Name:   "diary_order",
			...
			Column: Column{Table: "memo", Name: "payload"},
			Expressions: map[DialectName]string{
				DialectSQLite:   "COALESCE(CAST(json_extract(%s, '$.location.longitude') AS INTEGER), 0)",
				DialectMySQL:    "COALESCE(CAST(JSON_UNQUOTE(JSON_EXTRACT(%s, '$.location.longitude')) AS SIGNED), 0)",
				DialectPostgres: "COALESCE((%s->'location'->>'longitude')::bigint, 0)",
			},
		},
	}
	...
	envOptions := []cel.EnvOption{
		...
		cel.Variable("diary_status", cel.StringType),
		cel.Variable("diary_unlock_ts", cel.IntType),
		cel.Variable("diary_order", cel.IntType),
		nowFunction,
	}
