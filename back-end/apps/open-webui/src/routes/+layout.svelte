<script>
	...
	import { installActingUidFetch, getActingUserId } from '$lib/actingUser';
	...
	const setupSocket = async (enableWebsocket) => {
		const _socket = io(`${WEBUI_BASE_URL}` || undefined, {
			...
			auth: { acting_uid: getActingUserId() }
		});
		...
		_socket.on('connect', async () => {
			...
			const aid = getActingUserId();
			if (aid) {
				_socket.emit('user-join', { auth: { acting_uid: aid } });
			} else {
				console.warn('No acting user id; user-join event not emitted');
			}
		});
	...
	const resolveToolServer = (serverUrl) => {
		...
		if (toolServer) {
			...
			else if (auth_type === 'session') token = toolServer?.key ?? '';
		}
	...
	const channelEventHandler = async (event) => {
		...
		if (event.data?.type === 'channel:created') {
			const res = await getChannels('').catch(async (error) => {
				return null;
			});
