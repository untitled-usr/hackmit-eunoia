<script lang="ts">
	...
	const submitHandler = async () => {
		...
		if (tab === '') {
			...
			const res = await addUser(localStorage.token, {
				name: _user.name || undefined,
				password: _user.password || undefined,
				role: _user.role,
				profile_image_url: generateInitialsImage(_user.name || 'User')
			}).catch((error) => {
				toast.error(`${error}`);
			});
				...
				reader.onload = async (e) => {
					...
					for (const [idx, row] of rows.entries()) {
						...
						if (idx > 0) {
							if (
								columns.length === 3 &&
								['user', 'pending'].includes(columns[2].toLowerCase())
							) {
								const res = await addUser(localStorage.token, {
									name: columns[0] || undefined,
									password: columns[1] || undefined,
									role: columns[2].toLowerCase(),
									profile_image_url: generateInitialsImage(columns[0] || 'User')
								}).catch((error) => {
							...
							<div class="flex flex-col w-full mt-1">
								...
								<div class="flex-1">
									<input
										...
										placeholder={$i18n.t('Optional display name')}
										autocomplete="off"
									/>
								</div>
							</div>
							...
							<div class="flex flex-col w-full mt-1">
								...
								<div class="flex-1">
									<SensitiveInput
										...
										placeholder={$i18n.t('Leave empty for a random password')}
										autocomplete="off"
									/>
								</div>
							</div>
						{:else if tab === 'import'}
							<div>
								...
								<div class=" text-xs text-gray-500">
									ⓘ {$i18n.t(
										'Ensure your CSV file includes 3 columns in this order: Name, Password, Role.'
									)}
									<a
