...
def prompt_template(template: str, user: Optional[Any] = None) -> str:
    ...
    # Legacy placeholder: substitute user id (email field removed).
    template = template.replace(
        "{{USER_EMAIL}}", USER_VARIABLES.get("id", "Unknown")
    )
    template = template.replace("{{USER_ID}}", USER_VARIABLES.get("id", "Unknown"))
