                ...
                if type == "openapi":
                    ...
                    for spec in specs:
                        ...
                        if auth_type == "bearer":
                            ...
                            log.warning(
                                "Tool server auth_type 'session' is not supported; use bearer with a static key."
                            )
                            k = tool_server_connection.get("key", "")
                            if k:
...
async def get_terminal_tools(
    ...
    if auth_type == "bearer":
        ...
        log.warning(
            "Terminal server auth_type 'session' is not supported; use bearer with a static key."
        )
        k = connection.get("key", "")
        if k:
            headers["Authorization"] = f"Bearer {k}"
    elif auth_type == "system_oauth":
        cookies = request.cookies
