...
from open_webui.env import ACTING_USER_ID_HEADER, AUDIT_LOG_LEVEL, MAX_BODY_LOG_SIZE
from open_webui.utils.auth import get_user_from_acting_uid_header
...
class AuditLoggingMiddleware:
    ...
    async def _get_authenticated_user(self, request: Request) -> Optional[UserModel]:
        try:
            return get_user_from_acting_uid_header(request)
    ...
    def _should_skip_auditing(self, request: Request) -> bool:
        ...
        # Skip logging if the request has no acting user id
        if not (request.headers.get(ACTING_USER_ID_HEADER) or "").strip():
            return True
    ...
    async def _log_audit_entry(self, request: Request, context: AuditContext):
        try:
            ...
            user = user.model_dump(include={"id", "name", "role"}) if user else {}
