...
class UsersTable:
        ...
        with get_db_context(db) as db:
            user = UserModel(
                **{
                    ...
                    "username": username if username else id,
                    "oauth": oauth,
                }
            )
    ...
    def get_users(
        ...
        with get_db_context(db) as db:
            ...
            if filter:
                ...
                elif order_by in ("id", "email"):
                    # "email" sort key removed from schema; treat as id for older clients.
                    if direction == "asc":
    ...
    def has_any_admin(self, db: Optional[Session] = None) -> bool:
        with get_db_context(db) as db:
            return db.query(
                db.query(User).filter(User.role == "admin").exists()
            ).scalar()

    def get_system_admin_user(self, db: Optional[Session] = None) -> Optional[UserModel]:
        user = self.get_user_by_id(SYSTEM_ADMIN_USER_ID, db=db)
    ...
    def get_super_admin_user(self, db: Optional[Session] = None) -> Optional[UserModel]:
        return self.get_system_admin_user(db=db)
