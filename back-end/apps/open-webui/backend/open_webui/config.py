...
# Default True: Memos-style / X-Acting-Uid deployments often run with WEBUI_AUTH=False but still
# need POST /auths/register for self-service users. Set ENABLE_SIGNUP=false to disable.
...
# When True, public POST /api/v1/auths/register rejects new users while any user row exists
# (including the fixed system admin created at startup).
DISALLOW_USER_REGISTRATION = PersistentConfig(
    "DISALLOW_USER_REGISTRATION",
    "ui.disallow_user_registration",
    os.environ.get("DISALLOW_USER_REGISTRATION", "False").lower() == "true",
)
...
# Default "user": public register creates DEFAULT_USER_ROLE (never admin; admin is fixed at startup).
# Set DEFAULT_USER_ROLE=pending if you require admin activation before access.
