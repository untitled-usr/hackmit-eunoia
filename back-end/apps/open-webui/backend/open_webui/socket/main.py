...
def _get_header_from_environ(environ: dict, header_name: str) -> str:
    # python-engineio may expose headers in both HTTP_* keys and ASGI scope headers.
    env_key = f"HTTP_{header_name.upper().replace('-', '_')}"
    if environ.get(env_key):
        return str(environ.get(env_key))

    scope = environ.get("asgi.scope", {}) or {}
    for key, value in scope.get("headers", []) or []:
        try:
            if key.decode("utf-8").lower() == header_name.lower():
...
def _resolve_user_from_socket_auth(
    *, token: str | None = None, acting_uid: str | None = None
):
    uid = (acting_uid or "").strip()
    if uid:
        return Users.get_user_by_id(uid)
    return None


def _extract_acting_uid_from_connect(environ: dict, auth: dict | None) -> str:
    if auth and isinstance(auth, dict):
        uid = auth.get("acting_uid") or auth.get("user_id")
        if uid:
            return str(uid)

    header_uid = _get_header_from_environ(environ, ACTING_USER_ID_HEADER)
    if header_uid:
        return header_uid
    ...
    try:
        query = parse_qs(
            raw_query.decode("utf-8") if isinstance(raw_query, (bytes, bytearray)) else ""
        )
        return (
            (query.get("acting_uid", [""])[0] or query.get("user_id", [""])[0] or "")
            .strip()
        )
    except Exception:
        return ""
...
def _extract_token_and_uid_from_payload(data: dict) -> tuple[str | None, str]:
    ...
    if isinstance(auth, dict):
        ...
        acting_uid = str(auth.get("acting_uid") or auth.get("user_id") or "")

    if isinstance(data, dict):
        if not acting_uid:
            acting_uid = str(data.get("acting_uid") or data.get("user_id") or "")

    return token, acting_uid.strip()
...
async def connect(sid, environ, auth):
    ...
    acting_uid = _extract_acting_uid_from_connect(environ, auth)
    user = _resolve_user_from_socket_auth(token=token, acting_uid=acting_uid)
...
async def user_join(sid, data):
    token, acting_uid = _extract_token_and_uid_from_payload(data or {})
    user = _resolve_user_from_socket_auth(token=token, acting_uid=acting_uid)
    if not user:
        return
