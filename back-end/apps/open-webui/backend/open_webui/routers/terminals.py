...
from open_webui.env import ACTING_USER_ID_HEADER
...
async def proxy_terminal(
    ...
    if auth_type == "bearer":
        ...
        token_cred = getattr(request.state, "token", None)
        if token_cred is None:
            return JSONResponse(
                {"error": "Session token is required for this terminal connection"},
                status_code=401,
            )
        headers["Authorization"] = f"Bearer {token_cred.credentials}"
...
async def _resolve_authenticated_connection(ws: WebSocket, server_id: str):
    ...
    The client must send an auth message as its first frame:
    ``{"type": "auth", "user_id": "<id>"}`` or ``{"type": "auth", "acting_uid": "<id>"}``.
    ...
    try:
        ...
        if payload.get("type") != "auth":
            ...
            return None
        acting_uid = (payload.get("acting_uid") or payload.get("user_id") or "").strip()

        # Fallback to websocket header if message does not include user id
        if not acting_uid:
            acting_uid = (ws.headers.get(ACTING_USER_ID_HEADER) or "").strip()

        user = Users.get_user_by_id(acting_uid) if acting_uid else None
...
async def ws_terminal(
    ...
    Uses first-message auth. The client sends either a JWT token or acting user id
    in the first auth frame, then the proxy resolves the user and connects to the
    upstream terminal server.
    """
    await ws.accept()
