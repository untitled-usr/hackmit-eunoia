    ...
    try:
        if form_data.type == "mcp":
            if form_data.auth_type == "oauth_2.1":
                raise HTTPException(
                    status_code=400,
                    detail="MCP OAuth 2.1 authentication has been removed; use bearer, none, or a static key.",
                )
            client = None
            try:
                client = MCPClient()
                ...
                if form_data.auth_type == "bearer":
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail="auth_type 'session' is not supported; use bearer with a static key.",
                    )
                elif form_data.auth_type == "system_oauth":
                    raise HTTPException(
                        status_code=400,
                        detail="auth_type 'system_oauth' has been removed.",
                    )
                if token:
