...
from open_webui.constants import ERROR_MESSAGES, SYSTEM_ADMIN_USER_ID
...
async def update_user_by_id(
    ...
    if form_data.role == "admin" and user_id != SYSTEM_ADMIN_USER_ID:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=ERROR_MESSAGES.ADMIN_ROLE_NOT_ASSIGNABLE,
        )

    if user_id == SYSTEM_ADMIN_USER_ID:
